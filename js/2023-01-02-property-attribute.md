...
## 데이터 프로퍼티와 접근자 프로퍼티
### 데이터 프로퍼티
키와 값으로 구성된 일반적인 프로퍼티로 앞서 이야기한 프로퍼티 어트리뷰트를 갖는다. 
```js
const person = {
  name: 'Lee'
};
...
### 접근자 프로퍼티
자체적으로는 값을 갖지 않고 다른 데이터 프로퍼티의 값을 읽거나 저장할 때 호출되는 접근자 함수로 구성되어 있다. 접근자 함수는 getter setter 함수라고도 부른다. 프로퍼티 어트리뷰트는 다음 코드에서 확인할 수 있다.
```js
const person = {
  // 데이터 프로퍼티
  firstName: 'Ungmo',
...
### 객체 확장 금지 
`Object.isExtensible` 메소드를 통해 프로퍼티 추가를 금지 할 수 있다.
```js
...
### 객체 밀봉
`Object.seal` 메서드를 통해 프로퍼티 추가 및 삭제와 프로퍼티 어트리뷰트 재정의를 금지할 수 있다. 읽기와 쓰기만 가능하다.
```js
const person = { name: 'Lee' };
...
### 객체 동결
`Object.freeze` 메소드를 통해 프로퍼티 추가 및 삭제와 프로퍼티 어트리뷰트 재정의 금지, 프로퍼티 값 갱신 금지를 할 수 있다. 읽기만 가능하다.
```js
const person = { name: 'Lee' };
